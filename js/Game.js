...
spawnShark() {
    ...
    const interval = this.isEndlessMode ? Math.floor(CONFIG.SHARK.SPAWN_INTERVAL / 3) : CONFIG.SHARK.SPAWN_INTERVAL;
...
spawnBomb() {
    ...
    const interval = this.isEndlessMode ? Math.floor(CONFIG.BOMB.SPAWN_INTERVAL / 2) : CONFIG.BOMB.SPAWN_INTERVAL;
  ...
  startEndlessMode() {
    ...
    this.progressFill.style.background = 'linear-gradient(90deg, #ff6b6b, #ffa500)';
...
update() {
  ...
  cleanupOffscreenEnemies() {
    for (let i = this.enemies.length - 1; i >= 0; i--) {
      ...
      if (!enemy.isApex && (enemy.x < -200 || enemy.x > CONFIG.CANVAS_WIDTH + 200)) {
        enemy.destroy();
    ...
    for (let i = this.enemies.length - 1; i >= 0; i--) {
        ...
        if (this.player.isColliding(enemy)) {
            if (this.player.level >= enemy.level) {
                ...
                if (!this.isEndlessMode && this.player.score >= CONFIG.THRESHOLD.WIN) this.win();
            } else {
                enemy.openMouth();
